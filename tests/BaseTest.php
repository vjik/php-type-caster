...
class BaseTest extends TestCase
{
    ...
    {
        $filter = (new ValueFilter())->define(new NullFilter());
        ...
        $filter = (new ValueFilter())->define($nullFilter);
    ...
    {
        $filter = (new ValueFilter())->define(new IntFilter([]));
        $this->assertSame($filter->apply('1'), 1);

        $filter = (new ValueFilter())->define(new IntFilter(['skipOnEmpty' => false]));
        $this->assertSame($filter->apply(null), 0);

        $filter = (new ValueFilter())->define(new NullFilter(), new IntFilter());
        $this->assertNull($filter->apply(null));
    }

    public function testFloat()
    {
        $filter = (new ValueFilter())->define(new FloatFilter());
        ...
        $filter = (new ValueFilter())->define($floatFilter);
        $this->assertSame($filter->apply('1a1b1c2.12'), 11132.12);

        $filter = (new ValueFilter())->define(new FloatFilter(['skipOnEmpty' => false]));
        $this->assertSame($filter->apply(null), 0.0);

        $filter = (new ValueFilter())->define(new NullFilter(), new FloatFilter());
        $this->assertNull($filter->apply(null));
    }

    public function testString()
    {
        $filter = (new ValueFilter())->define(new StringFilter());
        $this->assertSame($filter->apply('42'), '42');
        $this->assertSame($filter->apply(42), '42');
        $this->assertSame($filter->apply(''), '');

        $filter = (new ValueFilter())->define(new NullFilter(), new StringFilter());
        $this->assertNull($filter->apply(''));
    }

    public function testTrim()
    {
        $filter = (new ValueFilter())->define(new StringFilter(['useTrim' => true]));
        $this->assertSame($filter->apply(' test '), 'test');
        $this->assertSame($filter->apply(42), '42');
        $this->assertNull($filter->apply(null));

        $filter = (new ValueFilter())->define(new StringFilter(['useTrim' => true, 'trimCharList' => 'a']));
        $this->assertSame($filter->apply('aba'), 'b');
    }

    public function testBool()
    {
        $filter = (new ValueFilter())->define(new BoolFilter());
        ...
        $filter = (new ValueFilter())->define(new BoolFilter(['trueValues' => [0]]));
        $this->assertTrue($filter->apply(0));

        $filter = (new ValueFilter())->define(new BoolFilter(['falseValues' => [1]]));
